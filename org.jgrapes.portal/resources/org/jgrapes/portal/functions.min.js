...
$(this).closest(".portlet").attr("data-portlet-id");JGPortal.sendDeletePortlet(a)}));b.includes("View")&&(l.prepend("<span class='ui-icon ui-icon-fullscreen portlet-expand'></span>"),l.find(".portlet-expand").on("click",function(){var a=$(this).closest(".portlet").attr("data-portlet-id");v(a)?m(a):JGPortal.sendRenderPortlet(a,"View",!0)}));var p=!1;$(".column").each(function(b){if(b>=w.length)return!1;var c=w[b];if(A(c,a,a))return $(this).find(".portlet").each(function(b){b=$(this);var l=b.attr("data-portlet-id");
if(!A(c,l,a))return b.before(f),p=!0,!1}),p||$(this).append(f),p=!0,!1});p||$(".column").first().prepend(f);n()}b=$(c);f.find(".portlet-header-text").text(b.attr("data-portlet-title"));c=f.find(".portlet-content");c.children().detach();c.append(b);d(b);e&&$("#portlet-tabs").tabs("option","active",0)}function n(){if(D){var a=[];$(".overview-panel").find(".column").each(function(b){var f=[];a.push(f);$(this).find("div.portlet[data-portlet-id]").each(function(a){f.push($(this).attr("data-portlet-id"))})});
...
a).remove();c.tabs("refresh");n()});$("#portal-session-suspended-dialog").dialog({autoOpen:!1,modal:!0});$(".column").sortable({connectWith:".column",handle:".portlet-header",cancel:".portlet-toggle",placeholder:"portlet-placeholder ui-corner-all",stop:function(a,b){n()}});JGPortal.sendPortalReady()};var t=new Set,u=[],r=new Set,B=!1;h.addMessageHandler("addPageResources",a);h.addMessageHandler("addPortletType",function(b,c,d,e,g){a(d,null,e);b=$('<li class="ui-menu-item"><div class="ui-menu-item-wrapper" data-portlet-type="'+
b+'">'+c+"</div></li>");$("#addon-menu-list").append(b)});var w=[[],[],[]],D=!1,z={};h.addMessageHandler("lastPortalLayout",function(a,b){w=a;0<r.size&&(JGPortal.lockMessageQueue(),B=!0)});h.addMessageHandler("reload",function(){window.location.reload(!0)});JGPortal.registerPortletMethod=function(a,b,c){var f=z[a];f||(f={},z[a]=f);f[b]=c};h.addMessageHandler("invokePortletMethod",function(a,b,c,d){(a=z[a])&&(c=a[c])&&c(b,d)});h.addMessageHandler("portalConfigured",function(){D=!0;n();$("body").faLoading("remove")});
JGPortal.findPortletPreview=function(a){a=$(".portlet[data-portlet-id='"+a+"']");if(1===a.length)return $(a[0])};var C=JGPortal.findPortletPreview;JGPortal.findPortletView=function(a){a=$("#portlet-tabs").tabs().find("> div[data-portlet-id='"+a+"']");if(1===a.length)return $(a[0])};var v=JGPortal.findPortletView;JGPortal.updatePortletViewTitle=function(a,b){var c=$("#portlet-tabs").tabs();a=c.find("> div[data-portlet-id='"+a+"']");0!==a.length&&(a.find("[data-portlet-title]").attr("data-portlet-title",
b),a=a.attr("id"),c=c.find("a[href='#"+a+"']"),c.empty(),c.append(b))};var E=1;h.addMessageHandler("updatePortlet",function(a,b,c,e,g){if("Preview"===b||"DeleteablePreview"===b)F(a,c,e,g);else if("View"===b){c=v(a);e=$(e);if(c)c.children().detach(),c.append(e);else{b=$("#portlet-tabs").tabs();E+=1;c="portlet-tabs-"+E;var f=$("<li><a href='@{href}'>@{label}</a> <span class='ui-icon ui-icon-close' role='presentation'>Remove Tab</span></li>".replace(/@\{href\}/g,"#"+c).replace(/@\{label\}/g,e.attr("data-portlet-title")));
b.find(".ui-tabs-nav").append(f);c=$("<div id='"+c+"'></div>");c.append(e);c.attr("data-portlet-id",a);b.append(c);b.tabs("refresh");n()}d(e);g&&m(a)}else"Edit"===b&&(g=$(e),g.dialog({modal:!0,width:"auto"}),g.attr("data-portlet-id",a),d(g))});h.addMessageHandler("deletePortlet",function(a){var b=v(a);b&&(b=b.closest(".ui-tabs-panel").remove().attr("id"),$("#portlet-tabs").tabs().find("li[aria-controls='"+b+"']").remove(),$("#portlet-tabs").tabs().tabs("refresh"));(a=C(a))&&a.remove();n()});h.addMessageHandler("retrieveLocalData",
function(a){var b=[];try{for(var c=0;c<localStorage.length;c++){var d=localStorage.key(c);if(!a.endsWith("/")){if(d!==a)continue}else if(!d.startsWith(a))continue;var f=localStorage.getItem(d);b.push([d,f])}}catch(H){q.error(H)}JGPortal.sendLocalData(b)});h.addMessageHandler("storeLocalData",function(a){try{for(var b in a){var c=a[b];"u"===c[0]?localStorage.setItem(c[1],c[2]):"d"===c[0]&&localStorage.removeItem(c[1])}}catch(G){q.error(G)}});h.addMessageHandler("createNotification",function(a,b){$(a).notification(b)});
